...
public class ResourceProviderProxyFactory extends ClassLoader {
    ...
    public ResourceProvider createResourceProviderProxy(String superProviderName, Class genericType){
        Class<?> clazz=createGenericClass(superProviderName,genericType);
    ...
    //创建泛型类，其中第二个参数为泛型参数类型
    public Class<?> createGenericClass(String superProviderName, Class genericType){
        String superClassName=superProviderName.replaceAll("\\.","/");
        System.out.println("父类名："+superClassName);
        //所创建类的类名
        String className=superClassName+genericType.getSimpleName()+"Proxy";
        System.out.println("子类名："+className);
        String genericClassName=String.format("L%s<L%s;>;",superClassName,genericType.getName().replaceAll("\\.","/"));
